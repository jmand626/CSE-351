...
int int_size() {
  ...
  int_ptr1 = int_array;
  //We do not need the '&' operator because an array is basically just a pointer to an array
  int_ptr2 = int_ptr1 + 1;
  //the next value

  //Cast as char because this is pointer aritmetic which means we scale by the datatype size -> Data type size of char is 1 ->No scaling here
...
void swap_ints(int* ptr1, int* ptr2) {
  ...
  *ptr1 = *ptr2;
  *ptr2 = temp;
  //Basic code where we have to swap the values of two variables. We must create a temp variable to hold in one of the values before swapping.
}
...
int change_value() {
  ...
  int* int_ptr2 = int_array + 5;
  //Because arrays are just pointers, we can add to an array like a pointer instead of using brackets ("[]")
  *int_ptr2 = 255 + 96;
  //We must add to get past limit
  return *(int_array + 5);
  //Returning the value of int_array[5];
}
...
int within_same_block(int* ptr1, int* ptr2) {
  ...
  //Unsigned integer without losing data
  uintptr_t p2 = (uintptr_t)ptr2;
  p1 = p1 >> 6;
  //We shift both of these 6 to the right as unsigned integers so the left 6 digits become 0 and therefore the same compared to each other so we compare the right two
  p2 = p2 >> 6;
...
int within_array(int* int_array, int size, int* ptr) {
  // Your code here
  char* value1 = (char*) int_array + 1;
  //Char pointer to one past the start to make sure we are in the array and to scale properly
  char* value2 = (char*) ptr;
  //Char pointer equals original
  char* value3 = (char*)(int_array + size)-1;
  //Char pointer to the end
  char firstSign = ((value2 - value1) >> 7);
  char secondSign = ((value3 - value2) >> 7);
  //With signed integers, figure out if differences are pos or neg by bit shifting to the right 7 times and only having 2 possible options

  
  //simple equality boolean check
  return firstSign == secondSign;
}

/*
    assert( ((int*)&student[4]) == 11 );
...
int string_length(char* s) {
  ...
  int length = 0;
  //start
  while(*s != '\0'){//While the piece of the string is not the null or terminating character
	length++;
	//increment length
	s++;
	//Next character
...
int endian_experiment(int* ptr) {
  ...
  // Your code here
  byte_ptr = (char*)ptr;
  //Cast as char for scaling safet
  //Start out on 77 because its little endian
  *byte_ptr = 0x77;
  //change value
  byte_ptr++;
  //go to next piece or 'byte'
  *byte_ptr = 0x5C;
  byte_ptr++;
  *byte_ptr = 0x05;
  byte_ptr++;
